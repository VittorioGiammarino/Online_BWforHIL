...
TrainingSet_tot = np.round(TrainingSet[:,:],3)
Labels_tot = labels[:]

nTraj = np.array([500, 1000, 2000, 4000, 5000, 10000, 20000]) #number of samples
...
for i in range(len(nTraj)):
    ...
    [trajBatch, controlBatch, OptionsBatch, 
     TerminationBatch, flagBatch] = BatchSim.HierarchicalStochasticSampleTrajMDP(max_epoch,nTraj_eval, seed)
    ...
    AverageRewardBatch = np.append(averageBatch,np.divide(np.sum(length_trajBatch),len(length_trajBatch)))
    STDBatch = np.std(length_trajBatch)
    ...
    [trajOnline, controlOnline, OptionsOnline, 
     TerminationOnline, flagOnline] = OnlineSim.HierarchicalStochasticSampleTrajMDP(max_epoch,nTraj_eval, seed)
    ...
    AverageRewardOnline = np.append(averageOnline,np.divide(np.sum(length_trajOnline),len(length_trajOnline))) 
    STDOnline = np.std(length_trajOnline)
